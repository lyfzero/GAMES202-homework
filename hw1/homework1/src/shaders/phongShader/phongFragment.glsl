...
float findBlocker( sampler2D shadowMap,  vec2 uv, float zReceiver ) {
  ...
  float depth = zReceiver;
  float filterSize = depth * depth * float(W_LIGHT);
  ...
  return (blockerCount < 1.0) ? 0.0 : blockerValue / blockerCount;
}
...
float PCSS(sampler2D shadowMap, vec4 coords){
  ...
  float penumbraSize = (zReceiver / avgBlockerDepth - 1.0) * float(W_LIGHT);
  float filterSize = float(NUM_SAMPLES) / 2048.0;
  // STEP 3: filtering
  float visibility = 0.0;
  poissonDiskSamples(coords.xy);
  for(int i = 0; i < NUM_SAMPLES; i++) {
    vec2 shadowCoord = coords.xy + poissonDisk[i] * penumbraSize * filterSize;
    float queryDepth = unpack(texture2D(shadowMap, shadowCoord));
    visibility += queryDepth < (zReceiver - bias) ? 0.0 : 1.0;
  }
...
float useShadowMap(sampler2D shadowMap, vec4 shadowCoord){
  ...
  float bias = 0.001;
  return queryDepth < (depth - bias) ? 0.0 : 1.0;
}
